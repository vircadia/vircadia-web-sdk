...
class Socket {
    ...
    clearConnections(): void {
        ...
        if (this.#_connectionsHash.size > 0) {
            Log.message("Clearing all remaining connections in Socket.", "networking");
    ...
    cleanupConnection(sockAddr: SockAddr): void {  // eslint-disable-line
        ...
        if (connectionErased && Socket.UDT_CONNECTION_DEBUG) {
            Log.message(`Socket.cleanupConnection called for connection to ${sockAddr.toString()}`, "networking");
        }
    }
    ...
    writePacketList(packetList: NLPacketList, sockAddr: SockAddr): number {
        ...
        if (packetList.getNumPackets() === 0) {
            Log.warning("Trying to send packet list with 0 packets, bailing.", "networking");
            return 0;
        }
    ...
    handleRemoteAddressChange = (previousAddress: SockAddr, currentAddress: SockAddr): void => {
        ...
        if (connection && connection.hasReceivedHandshake()) {
            ...
            Log.message(`Moved Connection class from ${previousAddress.toString()} to`
                        + currentAddress.toString(), "networking");
                    ...
                    if (packet.isReliable()) {
                        ...
                        if (!connection
                            ...
                            if (Socket.UDT_CONNECTION_DEBUG) {
                                Log.message(`Can't process packet: type ${NLPacket.typeInHeader(packet)}`
                                            + `, version ${NLPacket.versionInHeader(packet)}`, "networking");
    ...
    #writeReliablePacketList(packetList: NLPacketList, sockAddr: SockAddr): void {
        ...
        if (connection) {
            ...
            Log.message("Socket.writeReliablePacketList refusing to send packet list - no connection was created",
                "networking");
        }
    }
    ...
    #findOrCreateConnection(sockAddr: SockAddr, filterCreate = false): Connection | null {
        ...
        if (connection === undefined) {
            ...
            if (filterCreate && this.#_connectionCreationFilterOperator && !this.#_connectionCreationFilterOperator(sockAddr)) {
                ...
                if (Socket.UDT_CONNECTION_DEBUG) {
                    Log.message("Socket.findOrCreateConnection refusing to create Connection class for "
                        + sockAddr.toString() + " due to connection creation filter", "networking");
                }
                return null;
            }
            ...
            Log.message(`Creating new Connection class for ${sockAddr.toString()}`, "networking");
